...
class SimpleFilesystemTool(BaseTool):
    ...
    async def _list_directory(self, path: str) -> ExecutionResult:
        """디렉토리 목록 조회"""
    ...
    async def _create_directory(self, path: str) -> ExecutionResult:
        """디렉토리 생성"""
    ...
    async def _copy_item(self, source: str, destination: str) -> ExecutionResult:
        """파일/디렉토리 복사"""
        if not destination:
            return ExecutionResult(
                success=False,
    ...
    async def _move_item(self, source: str, destination: str) -> ExecutionResult:
        """파일/디렉토리 이동"""
        if not destination:
            return ExecutionResult(
                success=False,
    ...
    async def _delete_item(self, path: str) -> ExecutionResult:
        """파일/디렉토리 삭제"""
